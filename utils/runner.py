...
def run_trainer(cfg, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, cfg.max_epoch + 1):
        ...
        #additionally
        base_loss_names = ["plane_chamfer_loss", "classification_loss", 'chamfer_norm1_loss', 'chamfer_norm2_loss',
                          "plane_normal_loss", "repulsion_loss", "total_loss"]
        
        # check whether use refinement
        model_module = base_model.module if hasattr(base_model, 'module') else base_model
        if hasattr(model_module, 'use_refinement') and model_module.use_refinement:
            loss_names = base_loss_names + ["residual_loss", "svd_projection_loss"]
        else:
            loss_names = base_loss_names
        ...
        for idx, (model_ids, data) in enumerate(train_dataloader):
            ...
            if idx % 100 == 0:
                loss_info = f'[Epoch {epoch}/{cfg.max_epoch}][Batch {idx + 1}/{n_batches}] | '
                loss_info += f'Total: {losses["total_loss"]:.4f} | '
                loss_info += f'Chamfer: {losses["plane_chamfer_loss"]:.4f} | '
                loss_info += f'Class: {losses["classification_loss"]:.4f}'
    
                if 'residual_loss' in losses:
                    loss_info += f' | Residual: {losses["residual_loss"]:.4f}'
                if 'svd_projection_loss' in losses:
                    loss_info += f' | SVD: {losses["svd_projection_loss"]:.4f}'
    
                print_log(loss_info, logger=logger)
