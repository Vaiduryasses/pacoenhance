...
class PaCo(nn.Module):
    ...
    reconstruction head and additional classifier for plane predictions, additionally, with integrated refinement model.
    ...
    def __init__(self, config, **kwargs):
        ...
        self.use_refinement = getattr(config, 'use_refinement', False)
        ...
        #additionally
        self.last_refinement_results = None
    ...
    def get_loss(self, config, ret, class_prob, gt, gt_index, plane, plan_index, refinement_results=None):
        """
        ...
        Args:
            ...
            refinement_results: Refinement loss
        ...
        #additionally
        if self.use_refinement and self.last_refinement_results is not None:
            losses.update({
                "residual_loss": 0.0,
                "svd_projection_loss": 0.0
            })
        ...
        # additionally
        if self.use_refinement and self.last_refinement_results is not None:
            # compute SVD projection loss
            svd_projection_loss = self.svd_projection_loss_fn(self.last_refinement_results['displacement'])
            losses["svd_projection_loss"] = svd_projection_loss
            
            #compute residual loss 
            target_residual = torch.zeros_like(self.last_refinement_results['residual'])
            residual_loss = self.residual_loss_fn(self.last_refinement_results['residual'], target_residual)
            losses["residual_loss"] = residual_loss

        for k in losses.keys():
            if k.startswith('chamfer'):
                losses[k] /= batch_size
            elif k in ['residual_loss', 'svd_projection_loss']:
                # refinement损失已经在batch维度平均，不需要再除
                pass
            else:
                losses[k] /= size_total
        ...
        # additionally
        if self.use_refinement and self.last_refinement_results is not None:
            residual_loss_weight = getattr(config, 'residual_loss_weight', 1.0)
            svd_projection_loss_weight = getattr(config, 'svd_projection_loss_weight', 0.5)
            
            total_loss += (
                residual_loss_weight * losses["residual_loss"] +
                svd_projection_loss_weight * losses["svd_projection_loss"]
            )
    ...
    def forward(self, xyz):
        ...
        c = torch.cos(plane[:, :, 0])
        d = -plane[:, :, 2]
        initial_plane = torch.cat([a.unsqueeze(-1), b.unsqueeze(-1), c.unsqueeze(-1), d.unsqueeze(-1)], dim=-1)

        #additionally
        final_plane = initial_plane
        final_points = rebuild_points
        self.last_refinement_results = None  #reset

        if self.use_refinement:
            try:
                #apply refinement
                refinement_results = self.refinement_module(rebuild_points, initial_plane)
                
                # store refinement results for loss computing
                self.last_refinement_results = refinement_results
                
                # use refined results as final outputs
                final_points = refinement_results['refined_points']
                final_plane = refinement_results['refined_planes']
                
            except Exception as e:
                # if refinement failed
                print(f"Warning: Refinement failed with error: {e}. Using original results.")
                self.last_refinement_results = None

        ret = (final_plane, final_points)
